...
class ManageMenu(object):
    ...
    def display(self):
        ...
        self.varTraChanh = tk.StringVar()
        self.varNuocDua = tk.StringVar()
        self.varCaPhe = tk.StringVar()
        ...
        self.CaPheSua = tk.Checkbutton(self.f1top, text="Cà phê sữa", variable=self.var22, onvalue=1, offvalue=0, font=("arial", 18, 'bold'), command=lambda : self.x(), bg = '#7BED9F')
    ...
    def mn(self):
        if self.var133.get() == 1:
            ...
            self.varThachThuyTinh.set("0")
    def n(self):
        print(self.varTraChanh.get(),self.txtTraChanh.get())
        if self.var14.get() == 1:
            self.txtTraChanh.configure(state=NORMAL)
            self.varTraChanh.set("1")
